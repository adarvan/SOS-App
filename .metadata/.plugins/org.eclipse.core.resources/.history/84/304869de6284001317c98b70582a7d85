...
public class MainActivity extends Activity implements OnItemSelectedListener { 
	...
	View.OnClickListener a = new View.OnClickListener() {//le listener pour les bouttons pour SOS
		...
		public void onClick(View v) {
			...
			if(nombreBtn == 49)//si oui, affihcé qui a gagné
			{
				...
				if(yo!=0)
				Toast.makeText(MainActivity.this, "Le joueur " + String.valueOf(yo) + " a gagné", Toast.LENGTH_SHORT).show();//affiché le gagnant
				else if( yo == yo2)
				Toast.makeText(MainActivity.this, "C'est une égalité", Toast.LENGTH_SHORT).show();//s'il y a égalité
	...
	//ne marche pas autrement)
	{
		...
		.setMessage("Les joueurs prennent des tours pour ajouter ou un « S » ou un « O » sur n’importe quel carré " +
	...
	public void reset()//si reset est appeler, affiché un messagebox pour s'assuré que l'utilisateur est sur de son choix
	{
		...
		builder
		.setTitle("Recommencé le jeux?")
		.setMessage("Êtes-vous sur de vouloire recommencé le jeu??")
